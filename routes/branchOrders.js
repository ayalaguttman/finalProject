...
router.patch("/updateStatus/:userID", auth, async (req, res) => {
    if (!req.body.role) {
    ...
    try {
      ...
      if(req.tokenData.role == "admin"||req.tokenData.role == "worker"){
        let data = await BranchOrderModel.updateOne({_id:idEdit,worker_id:req.tokenData._id},{ status: req.body.status });
        res.json(data);

      }
      else{
        if(req.body.status=="Cancelled"){
      let data = await BranchOrderModel.updateOne({_id:idEdit,user_id:req.tokenData._id},{ status: req.body.status });
      res.json(data);
    }
    else{
